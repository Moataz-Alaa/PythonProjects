...
class Stack:
    ...
    def sortStackDescending(self):
        ...
        secondary = Stack()
        sorted = Stack()
        sorted.push(primary.pop())
        while(not (primary.isEmpty()) or not (secondary.isEmpty())):
            while(not (primary.isEmpty())):
                while((not (sorted.isEmpty()) and primary.peak() < sorted.peak())):
                    secondary.push(sorted.pop())
                if sorted.isEmpty():
                    sorted.push(primary.pop())
                while((not (primary.isEmpty()) and primary.peak() >= sorted.peak())):
                    sorted.push(primary.pop())
            while(not (secondary.isEmpty())):
                while(not (sorted.isEmpty()) and (secondary.peak() < sorted.peak())):
                    primary.push(sorted.pop())
                if sorted.isEmpty():
                    sorted.push(secondary.pop())
                while(not (secondary.isEmpty()) and (secondary.peak() >= sorted.peak())):
                    sorted.push(secondary.pop())
        return sorted


testStack = Stack([6,5,3,1,8,7,2,4])
print(testStack)
